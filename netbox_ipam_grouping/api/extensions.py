...
class ScopedIPAMFilterMixin(OwnershipQueryParamFilterMixin):
    ...
    owned_by_user=<pk>
        Restricts results to objects whose ipam_application custom field
        points to an Application owned by that user.

    cf_ipam_application= (present but empty)
    ...
    def filter_queryset(self, queryset):
        # If cf_ipam_application is explicitly in the request but has no value,
        # $application resolved to "" → no application selected → return empty.
        if (
            "cf_ipam_application" in self.data
            and not self.data.get("cf_ipam_application")
        ):
